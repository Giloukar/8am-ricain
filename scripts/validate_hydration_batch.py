#!/usr/bin/env python3
"""Validate reviewed LexiMind hydration batches and generate safe SQL updates."""
...
def validate_entry(entry: dict, index: int) -> list[str]:
    errors: list[str] = []
    required = ("word", "pos", "definition", "source_url")
    for key in required:
    ...
    if parsed.scheme != "https" or parsed.netloc != "fr.wiktionary.org" or not parsed.path.startswith("/wiki/"):
...
def build_sql(entries: list[dict], batch_name: str) -> str:
    chunks = [
        f"-- Generated from reviewed hydration batch {batch_name}.",
...
def main() -> int:
    ...
    parser.add_argument("input", help="Reviewed hydration JSON batch")
    ...
    if args.sql_out:
        ...
        print(f"valid {len(data)} entries; SQL written to {out}")
    else:
