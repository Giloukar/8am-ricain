...
window.SalonAccount={
 init:async function(){if(!window.supabase||!SALON_CONFIG.SUPABASE_URL||!SALON_CONFIG.SUPABASE_PUBLISHABLE_KEY){emit();return null}sb=window.supabase.createClient(SALON_CONFIG.SUPABASE_URL,SALON_CONFIG.SUPABASE_PUBLISHABLE_KEY);let r=await sb.auth.getSession();session=r.data.session;user=session?.user||null;sb.auth.onAuthStateChange(async(e,s)=>{session=s;user=s?.user||null;emit();if(e==="SIGNED_IN"&&user){try{await sb.functions.invoke("welcome-email")}catch(_){}}});emit();return user},
