...
async function nextTask(){let db=SalonAccount.db?.(),u=SalonAccount.user?.();if(!db||!u)return null;let r=await db.rpc("lexi_next_task");return r.error||!r.data?.length?null:{...shape(r.data[0]),recommended_exercise:r.data[0].recommended_exercise,evidence_types:Number(r.data[0].evidence_types||0),strong_successes:Number(r.data[0].strong_successes||0)}}
...
window.LexiExercises={mcq,review,nextTask,exerciseLabel};
})();